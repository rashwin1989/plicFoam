...
{
    double h_left, h_right, MW_left, MW_right;
    double delta = 1e-6;
    double *x_left, *x_right;
    int i;

    _NNEW2_(x_left, double, 2);
    _NNEW2_(x_right, double, 2);

    x_left[0] = (x[0] - delta)/(1-delta);
    x_left[1] = x[1]/(1-delta);
    MW_left = x_left[0]*MW[0] + x_left[1]*MW[1];
    plicFuncs::calc_h_from_T(x_left, P, T, n, Pc, Tc, w, MW, Tb, SG, H8, kij_T, Ta_kij, Tn_kij, nT_kij, kij, h_left);

    x_right[0] = (x[0]+delta)/(1+delta);
    x_right[1] = x[1]/(1+delta);
    MW_right = x_right[0]*MW[0] + x_right[1]*MW[1];
    plicFuncs::calc_h_from_T(x_right, P, T, n, Pc, Tc, w, MW, Tb, SG, H8, kij_T, Ta_kij, Tn_kij, nT_kij, kij, h_right);

    h[0] = 0.001*(h_right*MW_right*(1 + delta) - h_left*MW_left*(1 - delta))/(2*delta);

    x_left[0] = x[0]/(1-delta);
    x_left[1] = (x[1]-delta)/(1-delta);
    MW_left = x_left[0]*MW[0] + x_left[1]*MW[1];
    plicFuncs::calc_h_from_T(x_left, P, T, n, Pc, Tc, w, MW, Tb, SG, H8, kij_T, Ta_kij, Tn_kij, nT_kij, kij, h_left);

    x_right[0] = x[0]/(1+delta);
    x_right[1] = (x[1]+delta)/(1+delta);
    MW_right = x_right[0]*MW[0] + x_right[1]*MW[1];
    plicFuncs::calc_h_from_T(x_right, P, T, n, Pc, Tc, w, MW, Tb, SG, H8, kij_T, Ta_kij, Tn_kij, nT_kij, kij, h_right);

    h[1] = 0.001*(h_right*MW_right*(1 + delta) - h_left*MW_left*(1 - delta))/(2*delta);
...
int main(int argc, char *argv[])
{
    ...
    double Ta_kij, Tn_kij, P, T, h_tmp, dif, dif_num;
    int n, nT_kij, nT, nX, i, j, iT, iX, idx;
    double *kij, *kij_T, *h_par, *h_tmp_num;
    double *x, *y;
    ...
    f = fopen("h_T_x.dat+", "w");
    ft = fopen("constant/T_vector", "r");
    fx = fopen("constant/x_matrix", "r");
    iread = fscanf(ft, "%d", &nT);
    iread = fscanf(fx, "%d", &nX);
    iread = fscanf(fx, "%lf", &P);
    nX = nX/nT; // Should be an integer for the right inputs

    for(iT=0;iT<nT;iT++)
    {
        iread = fscanf(ft, "%lf", &T);
        Info << "T = " << T << " K, " << endl;
        plicFuncs::calc_kij_from_table(T, n, Ta_kij, Tn_kij, nT_kij, kij_T, kij);
        for(iX=0;iX<nX;iX++)
        {
            iread = fscanf(fx, "%lf %lf", &x[1], &x[0]); //x = [Xoil Xwater]
            ...
            calc_hpar_num(x, P, T, n, Pc, Tc, w, MW, Tb, SG, H8, kij_T, Ta_kij, Tn_kij, nT_kij, kij, h_tmp_num);
            plicFuncs::x2y(n, MW, x, y);
            dif = h_tmp - 1000.0*y[0]*h_par[0]/MW[0] - 1000.0*y[1]*h_par[1]/MW[1];
            dif_num = h_tmp - 1000.0*y[0]*h_tmp_num[0]/MW[0] - 1000.0*y[1]*h_tmp_num[1]/MW[1];
            fprintf(f,"%lf    %lf    %lf    %lf    %lf    %lf    %lf    %lf    %lf    %lf" "\n", T, x[1], h_par[0] , h_tmp_num[0], h_par[0] - h_tmp_num[0], h_par[1], h_tmp_num[1], h_par[1] - h_tmp_num[1], dif_num, dif);
        }
    }
