            ...
            forAll(pT1, fcI)
            {
                ...
                if(debug)
                {
                    os<< "Face: " << faceI << "  mag(Sf) = " << magSf_faceI << nl
                        << "magSf_ph1_own = " << magSf_ph1_own[faceI] << "  alpha1f_own = " << magSf_ph1_own[faceI]/magSf_faceI 
                        << "  magSf_ph1_nei = " << magSf_ph1_nei[faceI] << "  alpha1f_nei = " << magSf_ph1_nei[faceI]/magSf_faceI << nl
                        << "magSf_ph0_used = " << magSf_ph0_faceI << "  alpha0f_own = " << magSf_ph0_own[faceI]/magSf_faceI  << nl
                        << "  magSf_ph0_nei = " << magSf_ph0_nei[faceI] << "  alpha0f_nei = " << magSf_ph0_nei[faceI]/magSf_faceI << nl
                        << "face phase state for diffusion flux calculation: " << curPhaseState << nl
                        << "Own: " << faceOwn << nl
                        << "T1Own = " << T1Own << "  T1Nei = " << T1Nei << nl
                        << "T0Own = " << T0Own << "  T0Nei = " << T0Nei << nl
                        << "grad T0 = " << gradf_T0_faceI << "  grad T1 = " << gradf_T1_faceI << nl
                        << "diffFlux_T1 = " << diffFlux_T1_faceI << "  limiter_ph1 = " << diffFlux_limiter_ph1_faceI << nl
                        << "diffFlux_T0 = " << diffFlux_T0_faceI << "  limiter_ph0 = " << diffFlux_limiter_ph0_faceI << nl
                        << endl;
                }
...
)
{
    ...
    //  Print
    Info << "calc_T_from_h called." << endl;
    ...
    T = TNew;
    //  Print
    Info << "calc_T_from_h returned with " << iter << " iterations."<< endl;
}
...
)
{
    ...
    {        
        ...
        TNew = TOld - F/dF;
        if(TNew > Tmax) TNew = 0.3*TOld + 0.7*Tmax;
        if(TNew < Tmin) TNew = 0.3*TOld + 0.7*Tmin;
...
)
{
    ...
    double TLowerLimit, TUpperLimit;
    ...
    if(mag(diffFlux) > SMALL)
    {
        ...
        for(iter=0; iter<MAX_ITER_T; iter++)
        {
            ...
            if(TNew < TLowerLimit) TNew = 0.7*TOld + 0.3*TLowerLimit;
            else if(TNew > TUpperLimit) TNew = 0.7*TOld + 0.3*TUpperLimit;
        ...
        if(diffFlux > 0)
        {
            if(TOwn > TNei)
            {
                diffFlux_max_1 = mag(0.5*VOwn*(HOwn - alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp));
                ...
                diffFlux_max_2 = mag(0.5*VOwn*(HOwn - alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp));
                diffFlux_max_3 = mag(0.5*VNei*(alphaNei*rhoNei*hNei_tmp/MWNei_tmp - HNei));
        ...
        else
        {
            if(TOwn < TNei)
            {
                diffFlux_max_1 = mag(0.5*VNei*(HNei - alphaNei*rhoNei*hNei_tmp/MWNei_tmp));
                calc_kij_from_table(TMAX, n, Ta_kij, Tb_kij, nT_kij, kij_T, kij);
                ...
                diffFlux_max_2 = mag(0.5*VOwn*(alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp - HOwn));
                diffFlux_max_3 = mag(0.5*VNei*(HNei - alphaNei*rhoNei*hNei_tmp/MWNei_tmp));
                diffFlux_max = min(diffFlux_max_1, min(diffFlux_max_2, diffFlux_max_3));
            }
            else
            {
...
void calc_intfcDiffFlux_limiter_T
(
    const scalar& alpha1,
    const scalar& V_cell,
    const scalar& rhoOwn,
...
)
{
    ...
    scalar diffFlux = -Hs1*V_cell; // diffFlux is positive from Own to Nei
    scalar VOwn, alphaOwn, VNei, alphaNei; 
    double TMAX, TMIN;
    VOwn = V_cell; VNei = V_cell;
    alphaOwn = alpha1; alphaNei = 1 - alpha1;
    TMAX = min(T1MAX,T0MAX);
    TMIN = max(T1MIN, T0MIN);
    ...
    if(mag(diffFlux) > SMALL)
    {
        ...
        if(diffFlux > 0)
        {
            if(TOwn > TNei)
            {
                diffFlux_max_1 = mag(1*VOwn*(HOwn - alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp));
                calc_kij_from_table(TMIN, n, Ta_kij, Tb_kij, nT_kij, kij_T, kij);
                calc_v_h_(&P, &TMIN, xOwn_tmp, &n, Pc, Tc, w, MW, Tb, SG, H8, kij, &vOwn_tmp, &hOwn_tmp);
                calc_kij_from_table(TMAX, n, Ta_kij, Tb_kij, nT_kij, kij_T, kij);
                calc_v_h_(&P, &TMAX, xNei_tmp, &n, Pc, Tc, w, MW, Tb, SG, H8, kij, &vNei_tmp, &hNei_tmp);

                diffFlux_max_2 = mag(1*VOwn*(HOwn - alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp));
                diffFlux_max_3 = mag(1*VNei*(alphaNei*rhoNei*hNei_tmp/MWNei_tmp - HNei));
                diffFlux_max = min(diffFlux_max_1, min(diffFlux_max_2, diffFlux_max_3));
            }
            else
            {
        ...
        else
        {
            if(TOwn < TNei)
            {
                diffFlux_max_1 = mag(1*VNei*(HNei - alphaNei*rhoNei*hNei_tmp/MWNei_tmp));
                calc_kij_from_table(TMAX, n, Ta_kij, Tb_kij, nT_kij, kij_T, kij);
                calc_v_h_(&P, &TMAX, xOwn_tmp, &n, Pc, Tc, w, MW, Tb, SG, H8, kij, &vOwn_tmp, &hOwn_tmp);
                calc_kij_from_table(TMIN, n, Ta_kij, Tb_kij, nT_kij, kij_T, kij);
                calc_v_h_(&P, &TMIN, xNei_tmp, &n, Pc, Tc, w, MW, Tb, SG, H8, kij, &vNei_tmp, &hNei_tmp);
                diffFlux_max_2 = mag(1*VOwn*(alphaOwn*rhoOwn*hOwn_tmp/MWOwn_tmp - HOwn));
                diffFlux_max_3 = mag(1*VNei*(HNei - alphaNei*rhoNei*hNei_tmp/MWNei_tmp));
                diffFlux_max = min(diffFlux_max_1, min(diffFlux_max_2, diffFlux_max_3));
            }
            else
            {
...
)
{
    ...
    int i;
    double erf1, x_delta1, delta1_cellI, erf0, x_delta0, delta0_cellI, pi, two_sqrtPi;
    scalar alpha1_cellI, A_intfc_cellI, dn1, dn0, Teff1, Teff0, Ts_cellI, lambda1_cellI, lambda0_cellI, V_cellI;
    vector nf, C_intfc_cellI;
    ...
    {
        alpha1_cellI = alpha1Cells[cellI];        
        Ts_cellI = alpha1_cellI*T1Cells[cellI] + (1 - alpha1_cellI)*T0Cells[cellI];

        //        Info << "calc_Ts: alphaCell = " << alpha1_cellI << " ALPHA_2PH_MIN = " << ALPHA_2PH_MIN << " ALPHA_2PH_MAX = " << ALPHA_2PH_MAX << endl;
        ...
        if(alpha1_cellI > ALPHA_2PH_MIN && alpha1_cellI < ALPHA_2PH_MAX)
        {
            ...
            //phase-0
            //ensure nf direction is into the phase; then reverse nf again for Js0 calculation            
            ...
            // Calculate Hs and Ts
            lambda1_cellI = lambda1.internalField()[cellI];
            lambda0_cellI = lambda0.internalField()[cellI];
            V_cellI = cellVol[cellI];
            Hs1.internalField()[cellI] =  A_intfc_cellI*lambda1_cellI*(Ts_cellI - Teff1)/dn1/V_cellI;
            Hs0.internalField()[cellI] = - Hs1.internalField()[cellI];
            Ts_cellI = (lambda1_cellI*Teff1/dn1 + lambda0_cellI*Teff0/dn0)/(lambda1_cellI/dn1 + lambda0_cellI/dn0);
            TsCells[cellI] = Ts_cellI;

            //            Info << "T1 = " << Teff1 << " T0 = " << Teff0 << " dn1 = " << dn1 << " dn0 = " << dn0 << " Ts = " << Ts_cellI << " alpha1 = " << alpha1Cells[cellI] <<  endl;
            //            Info << "Lambda1 = " << lambda1_cellI << " lambda0 = " << lambda0_cellI << endl;
...
)
{
    ...
    double vis, cond, V, CvIG;
    double *x_tmp, *x;
    double mu0_default = 3.5e-5;

    _NNEW_(x_tmp, double, n);
    _NNEW_(x, double, n);

    // Mole-fraction weighted mu and cond
    mu = 0;
    lambda = 0;
    y2x(n, MW, y, x);
    for(i=0; i<n; i++)
    ...
    /*    
    // Using Chung correlations for mixtures
    y2x(n, MW, y, x_tmp);
    calc_v_cvig_(&P,&T,x_tmp,&n,Pc,Tc,w,MW,Tb,SG,H8,kij,&V,&CvIG);
    vis_n_cond_(&P,&T,&n,Pc,Tc,Vc,w,MW,k,dm,x_tmp,&CvIG,&V,&cond,&vis);
    if(vis < mu0_default)
    ...
    forAll(alpha1Cells, cellI)
    {
        ...
        if(alpha1_cellI > ALPHA_2PH_MIN && alpha1_cellI < ALPHA_2PH_MAX)
        {
            ...
            for(i=0; i<n; i++)
            {
                ...
                //                Hs1_cellI += max(mSi,0)*hpar0[i].internalField()[cellI] + min(mSi,0)*hpar1[i].internalField()[cellI];
                Hs1_cellI += mSi*(hpar0[i].internalField()[cellI] + hpar1[i].internalField()[cellI])/2;
            }
