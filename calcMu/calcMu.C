...
Description
    Generate lookup table for viscosity
...
int main(int argc, char *argv[])
{
    ...
    double Ta_kij, Tn_kij, P, Ta, Tn, T, dT, Ya, Yn, Y, dY, V, CvIG, mu_tmp, cond;
    int n, nT_kij, nT, nY, i, j, iT, iY, idx, bKijSet;
    double *kij, *kij_T;
    double *y, *x;
    ...
    f = fopen("constant/calcMu_input.dat+", "r");
    ...
    for(iT=0; iT<nT; iT++)
    {
        ...
        for(iY=0; iY<nY; iY++)
        {
            ...
            calculate_kij_from_table_(&bKijSet,&T,&n,kij);
            calc_v_cvig_(&P,&T,x,&n,Pc,Tc,w,MW,Tb,SG,H8,&V,&CvIG);
            vis_n_cond_(&P,&T,&n,Pc,Tc,Vc,w,MW,k,dm,x,&CvIG,&V,&cond,&mu_tmp);
