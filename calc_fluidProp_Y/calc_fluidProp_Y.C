...
int main(int argc, char *argv[])
{
    ...
    f = fopen("prop.dat+", "w");
    T = Tmin;
    fprintf(f,"%s    %s   %s   %s   %s   %s   %s   %s   %s    %s" "\n", "T [K]", "x_water", "Cp [J/mol]", "H [J/kg-mixture]", "h-par-water [J/mol-water]", "h-par-oil [J/mol-oil]",  "Visc [Pa.s]", "Cond [W/m/K]", "Rho [kg/m3]", "Diff [SI]*10^8");
    ...
    for(iT=0;iT<nT;iT++)
    {
        ...
        x[0] = Xmin;
        x[1] = 1- x[0];
        double y0, y1, mw0, mw1, dY;
        y0 = x[0]; y1 = x[1]; // Mass fractions of oil and water respectively
        mw0 = MW[0]; mw1 = MW[1];
        dY = dX;
        for(iX=0;iX<nX;iX++)
        {
            x[0] = y0/mw0/(y0/mw0 + y1/mw1); // Mole fractions from mass fractions
            x[1] = y1/mw1/(y0/mw0 + y1/mw1);
            Info << "yWater = " << y1 << endl;
            //Mu and K
            ...
            fprintf(f,"%lf    %lf   %lf   %lf   %lf   %lf %lf    %lf    %lf    %lf    %lf    %lf    %lf" "\n", T, x[1], Cp_tmp, h_tmp, h_par[1], h_par[0],  mu_tmp, cond_tmp, rho_tmp, D_2_comp, Dij[0]*100000000, Dij[2]*100000000, Dij[3]*100000000);
            y0 = min(1, y0 + dY);
            y1 = 1- y0;
        }
        T = T + dT;
